...
| Everything the repository builds or checks with is Go | The Claude Desktop bundle is packed by `@anthropic-ai/mcpb`, which is Node, run through `npx` at RELEASE time only | It is the official packer and it validates the manifest against the published schema before writing. Re-implementing that in Go means carrying our own copy of a schema we do not own and trusting it — the failure mode being a bundle that packs here and is refused by the client, which is exactly what the validation was for. Nothing Node enters the repository, the module graph or the binary; the version is pinned exactly, like every other tool the release fetches. The orchestration around it IS Go, under `scripts/gates`, so the staging, the one-match globs and the manifest's version injection are held to the same standard as everything else here |
