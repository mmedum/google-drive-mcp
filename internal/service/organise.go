...
func (s *Service) CopyFile(ctx context.Context, in CopyFileInput) (*Result, error) {
	...
	if in.CopyComments {
		...
		notes = append(notes, "Drive was asked to bring the comment threads along. It does not say "+
			"whether it did: list_comments on the copy shows what came across, though it can lag a "+
			"copy by a moment. Anything that did come across is what other people wrote, now "+
